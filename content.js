// content.js - Final Version with Debug Logging
...
function showWarningModal(url) {
  console.log('QuickPhish: Showing warning for URL:', url);
  ...
  modal.querySelector('#quickphish-proceed').addEventListener('click', () => {
    console.log('QuickPhish: User proceeded with phishing URL:', url);
    document.body.removeChild(modal);
    window.open(url, '_blank');
  });

  modal.querySelector('#quickphish-cancel').addEventListener('click', () => {
    console.log('QuickPhish: User stayed safe from phishing URL:', url);
    document.body.removeChild(modal);
  });
...
document.addEventListener('click', async (event) => {
  console.log('QuickPhish: Click event detected');
  ...
  while (target && target.tagName !== 'A') {
    ...
    if (!target) {
      console.log('QuickPhish: No anchor tag found');
      return;
    }
  }

  const url = target?.href;
  console.log('QuickPhish: Detected URL:', url);
  if (!url) {
    console.log('QuickPhish: No URL found');
    return;
  }

  if (!isInEmailBody(target)) {
    console.log('QuickPhish: Link not in email body');
    return;
  }

  // Skip trusted domains
  if (TRUSTED_DOMAINS.some(domain => new URL(url).hostname.includes(domain))) {
    console.log('QuickPhish: URL in trusted domains, skipping');
    return;
  }
  ...
  console.log('QuickPhish: Checking URL safety:', url);
  ...
  try {
    ...
    console.log('QuickPhish: Received response:', response);
